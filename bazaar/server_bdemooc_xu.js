...
io.sockets.on('connection', function (socket) { //This sucket parameter is the socket that a clint made and made a connection throught it to the server 
    ...
    socket.on('chatroom_end_of_conversation_button', function(room)
	{
    ...
    socket.on('essaySend', function (data) 
	{
        ...
        //Behzad I dont know what this line will do
		io.sockets.in(socket.room).emit('updatechat', socket.username, data);
	});
	...
	socket.on('disconnect', function()
	{
	    ...
	    if(socket.room in usernames && socket.username in usernames[socket.room])
	    {
            ...
            if(usernames[socket.room])
            {
                ...
                if (socket.type === 'chatroom'){
                    chatroom_end_of_conversation_button(socket);
                    // logMessage(socket, "The room is locked.", "INFO", 'INFO');
                    // io.sockets.in(socket.room).emit('updatechat', 'INFO', 'The room is locked.');
                    
                    // logMessage(socket, 'You can save the conversation by clicking on the "Download Chat Log" button or come back to the login page.','INFO', 'INFO');
                    // io.sockets.in(socket.room).emit('updatechat', 'INFO', 'You can save the conversation by clicking on the "Download Chat Log" button or come back to the login page.');
